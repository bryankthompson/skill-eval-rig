...
def _parse_judge(raw):
    """Pull the last shallow JSON object carrying a pass/fail verdict; fall back to a bare word.
    Returns (verdict, confidence) or (None, 0.0) when unparseable (→ the grader abstains)."""
    for blob in reversed(re.findall(r"\{[^{}]*\}", raw or "")):
        try:
            d = json.loads(blob)
        except Exception:
            continue
